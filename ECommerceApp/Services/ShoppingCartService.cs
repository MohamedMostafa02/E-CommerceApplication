...
namespace ECommerceApp.Services
{
    public class ShoppingCartService
    {
        ...
        public async Task<ApiResponse<CartResponseDTO>> GetCartByCustomerIdAsync(int  customerId)
        {
            try
            {
                ...
                var cart = await _context.Carts
                    ...
                    .FirstOrDefaultAsync(c => c.CustomerId == customerId && !c.IsCheckOut); 
            ...
            catch (Exception ex)
            {
                ...
                return new ApiResponse<CartResponseDTO>(500, $"An unexpected error occured while processing your request, Error: {ex.Message}");
            }
        }

        // Adds a product to the customer's cart.
        // Creates an active cart if doesn't exists.
        // If the product already exists in the cart, it's quqntity is updated.

        public async Task<ApiResponse<CartResponseDTO>> AddToCartAsync(AddToCartDTO addToCartDTO)
        {
            try
            {
                // Retrieves the product fro the database using the provided ProductId.
                var product = await _context.Products.FindAsync(addToCartDTO.ProductId);    

                if(product == null)
                {
                    // Return 404 if the product not found.
                ...
                // Reterieve an active cart for the customer (include related CartItems and Products).

                var cart = await _context.Carts
                    .Include(c => c.CartItems)
                    .ThenInclude(ci => ci.Product)
                    .FirstOrDefaultAsync(c => c.CustomerId == addToCartDTO.CustomerId && !c.IsCheckOut);
                ...
                if(existingCartItem != null)
                {
                    ...
                    {
                        int remainingStock =  product.StockQuantity - existingCartItem.Quantity;

                        // Check if there is no left items
                        if(remainingStock <= 0)
                        {
                            return new ApiResponse<CartResponseDTO>(400,$"This product is out of stock." +
                                           $" You already have {existingCartItem.Quantity} in your cart.");
                        }
                        return new ApiResponse<CartResponseDTO>(400,$"Only {remainingStock} more units available in stock." +
                                        $" You currently have {existingCartItem.Quantity} in your cart.");
        ...
        public async Task<ApiResponse<CartResponseDTO>> UpdateCartItemAsync(UpdateCartItemDTO updateCartItemDTO)
        {
            try
            {
                // Retrieves the active cart for the customer along with cart items product details.

                var cart = await _context.Carts
                    .Include(c => c.CartItems)
                    .ThenInclude(ci => ci.Product)
                    .FirstOrDefaultAsync(c => c.CustomerId == updateCartItemDTO.CustomerId && !c.IsCheckOut);
                ...
                // Ensure the updated quantity doesn't exceed the available product stock.
                ...
                // Update the cart item's quantity and recalculate it's total price.
                cartItem.Quantity = updateCartItemDTO.Quantity;
                cartItem.TotalPrice = (cartItem.UnitPrice - cartItem.Discount) * cartItem.Quantity;
                cartItem.UpdatedAt = DateTime.UtcNow;

                // Mark teh cart item as updated.
                ...
                // Reload the updated cart with it's items.
                cart = await _context.Carts
                    .Include(c => c.CartItems)
                    .ThenInclude(ci => ci.Product)
                    .FirstOrDefaultAsync(c => c.Id == cart.Id) ?? new Cart();
        ...
        // Remove a specific item from the customers cart
        public async Task<ApiResponse<CartResponseDTO>> RemoveCartItemAsync(RemoveCartItemDTO removeCartItemDTO)
        {
            try
            {
                // Retreive the active cart along with its items and product details.
                var cart = await _context.Carts
                    .Include(c => c.CartItems)
                    .ThenInclude(ci => ci.Product)
                    .FirstOrDefaultAsync(c => c.CustomerId == removeCartItemDTO.CustomerId && !c.IsCheckOut);
        ...
        public async Task<ApiResponse<ConfirmationResponseDTO>> ClearCartAsync(int customerId)
        {
            try
            {
                // Retrieve the active cart along with it's items.
                var cart = await _context.Carts
                    .Include(c => c.CartItems)
                    .FirstOrDefaultAsync(c => c.CustomerId == customerId && !c.IsCheckOut);
        ...
        {
            // Map each CartItem entity to it's corresponding CartItemResponseDTO.
            ...
            // Create and return the final CartResponseDTO with all details and calcualted totals.

            return new CartResponseDTO
            {
                Id = cart.Id,
                CustomerId = cart.CustomerId,
                IsCheckedOut = cart.IsCheckOut,
