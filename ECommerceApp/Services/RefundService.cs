...
namespace ECommerceApp.Services
{
    public class RefundService
    {
        ...
        public async Task<ApiResponse<List<PendingRefundResponseDTO>>> GetEligibleRefundAsync()
        {
            try
            {
               ...
               && c.Order.Payment.PaymentMethod.ToUpper() != "COD")
            ...
            {
                return new ApiResponse<List<PendingRefundResponseDTO>>(500, $"An unexpected error occured while processing your request, Error: {ex.Message}");
            }
        }

        // The ProcessRefundAsync method in the service will handles only those cancellations that are approved
        // ad have no entry in the refunds table
        public async Task<ApiResponse<RefundResponseDTO>> ProcessRefundAsync(RefundRequestDTO refundRequestDTO)
        {
            try
            {
                ...
                var cancellation = await _context.Cancellations
                    ...
                    .FirstOrDefaultAsync(c => c.Id == refundRequestDTO.CancellationId);
                ...
                var existingRefund = await _context.Refunds
                     .FirstOrDefaultAsync(r => r.CancellationId == refundRequestDTO.CancellationId);
                ...
                if (payment == null || payment.PaymentMethod.ToUpper() == "COD")
                {
                ...
                {
                    CancellationId = refundRequestDTO.CancellationId,
                    PaymentId = payment.Id,
                    Amount = computedRefundAmount,
                    RefundMethod = refundRequestDTO.RefundMethod.ToString(),
                    RefundReason = refundRequestDTO.RefundReason,
                    Status = RefundStatus.Pending,
                    InitiatedAt = DateTime.UtcNow,
                    ProcessedBy = refundRequestDTO.ProcessedBy,
                ...
                // Immediately try processing via the simulated payment gatewway.
        ...
        public async Task<ApiResponse<ConfirmationResponseDTO>> UpdateRefundStatusAsync(RefundStatusUpdateDTO statusUpdateDTO)
        {
            try
            {
               ...
               .ThenInclude(o => o.Customer)
               .Include(r => r.Payment)
               .FirstOrDefaultAsync(r => r.Id == statusUpdateDTO.RefundId);
                ...
                refund.RefundMethod = statusUpdateDTO.RefundMethod.ToString();
                refund.Status = RefundStatus.Completed;
                refund.TransactionId = statusUpdateDTO.TransactionId;
                refund.CompletedAt = DateTime.UtcNow;
                refund.ProcessedBy = statusUpdateDTO.ProcessedBy;
                refund.RefundReason = statusUpdateDTO.RefundReason;
                ...
                if (refund.Cancellation?.Order?.Customer != null && !string.IsNullOrEmpty(refund.Cancellation.Order.Customer.Email))
                {
                    await _emailService.SendEmailAsync(
                        ...
                        $"Your Refund Has Been Processed Successfully, Order #" +
                        $"{refund.Cancellation.Order.OrderNumber}",
                        GenerateRefundSuccessEmailBody(refund, refund.Cancellation.Order.OrderNumber,
                        refund.Cancellation),
                        IsBodyHtml: true);
                }
        ...
        public async Task<ApiResponse<RefundResponseDTO>> GetRefundByIdAsync(int id)
        {
            ...
            {
                return new ApiResponse<RefundResponseDTO>(500, $"An expected error occured while processing your request, Error: {ex.Message}");
            }
        }
        ...
        public async Task<ApiResponse<List<RefundResponseDTO>>> GetAllRefundsAsync()
        {
            ...
            {
                return new ApiResponse<List<RefundResponseDTO>>(500, $"An unexpected error occured while processing your request, Error: {ex.Message}");
            }
        }
        ...
        public string GenerateRefundSuccessEmailBody(Refund refund, string orderNumber, Cancellation cancellation)
        {
            ...
            ? TimeZoneInfo.ConvertTimeFromUtc(refund.CompletedAt.Value, egyptZone).ToString("dd MMM yyyy HH:mm:ss")
            : "N/A";
            return $@"
<html>
...
<tr>
<td style='border: 1px solid #ddd; padding: 8px;'>Completed At</td>
