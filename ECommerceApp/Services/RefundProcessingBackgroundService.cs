...
namespace ECommerceApp.Services
{
    public class RefundProcessingBackgroundService : BackgroundService
    {
        ...
        private readonly TimeSpan _internal = TimeSpan.FromMinutes(5);
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingtoken)
        {
            ...
            {
                await ProcessPendingAndFailedRefundAsync(stoppingtoken);
                await Task.Delay(_internal,stoppingtoken);
            }
        }


        private async Task ProcessPendingAndFailedRefundAsync(CancellationToken cancellationToken)
        {
            ...
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                foreach(var refund in refunds)
                {
                    ...
                    if(gatewayresponse.IsSuccess)
                    {
                        ...
                        {
                            await emailService.SendEmailAsync(refund.Cancellation.Order.Customer.Email,
                                $"Your Refund Has Been Processed Successfully, Order # {refund.Cancellation.Order.OrderNumber}", refundService.GenerateRefundSuccessEmailBody(refund,refund.Cancellation.Order.OrderNumber, refund.Cancellation), IsBodyHtml: true);
                        }
                    }
                    else
                    {
