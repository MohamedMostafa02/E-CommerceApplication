...
namespace ECommerceApp.Services
{
    public class CancellationService
    {
        ...
        public async Task<ApiResponse<CancellationResponseDTO>> RequestCancellationAsync(CancellationRequestDTO cancellationRequestDTO)
        {
            try
            {
                // Validate order existence with it's items and product details (read-only)
                var order = await _context.Orders
                    .AsNoTracking()
                    .FirstOrDefaultAsync(o => o.Id == cancellationRequestDTO.OrderId && o.CustomerId == cancellationRequestDTO.CustomerId);
                ...
                {
                    return new ApiResponse<CancellationResponseDTO>(400, "Order isn't eligible for cancellation.");
                }

                // Check if a cancellation request for the order already exists.
                var existingCancellation = await _context.Cancellations
                    .AsNoTracking()
                    .FirstOrDefaultAsync(c => c.OrderId == cancellationRequestDTO.OrderId);
                ...
                {
                    OrderId = cancellationRequestDTO.OrderId,
                ...
                // Mapping from Cancellation fot CancellationResponseDTO
        ...
        // Retrieves a cancellation request by it's ID.
        public async Task<ApiResponse<CancellationResponseDTO>> GetCancellationByIdAsync(int id)
        {
            ...
            {
                return new ApiResponse<CancellationResponseDTO>(500, $"An unpexected error occured: {ex.Message}");
            }
        }

        // Updates the status of the cancellation request (approval/rejection) by an administrator.
        // Aslo handles order status update and stock restoration if approved.
        public async Task<ApiResponse<ConfirmationResponseDTO>>  UpdateCancellationStatusAsync(CancellationStatusUpdateDTO cancellationStatusUpdateDTO)
        {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    var cancellation = await _context.Cancellations
                        ...
                        .FirstOrDefaultAsync(c => c.Id == cancellationStatusUpdateDTO.CancellationId);
                    ...
                    cancellation.Status = cancellationStatusUpdateDTO.Status;
                    cancellation.ProcessedAt = DateTime.UtcNow;
                    cancellation.ProcessedBy = cancellationStatusUpdateDTO.ProcessedBy;
                    cancellation.Remarks = cancellationStatusUpdateDTO.Remarks;

                    if (cancellationStatusUpdateDTO.Status == CancellationStatus.Approved)
                    {
                        // Update the order status to Canceled
                        cancellation.Order.OrderStatus = OrderStatus.Canceled;
                        cancellation.CancellationCharges = cancellationStatusUpdateDTO.CancellationCharges;
                    ...
                    else if(cancellationStatusUpdateDTO.Status == CancellationStatus.Rejected)
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    return new ApiResponse<ConfirmationResponseDTO>(500, $"An unexpected error occured: {ex.Message}");
                }
            }
        }
        ...
        public async Task<ApiResponse<List<CancellationResponseDTO>>> GetAllCancellationsAsync()
        {
            try
            {
                ...
                var cancellationList = cancellation.Select(c => new CancellationResponseDTO
                {
                    Id = c.Id,
                    OrderId = c.OrderId,
                    Reason = c.Reason,
            ...
            {
                return new ApiResponse<List<CancellationResponseDTO>>(500, $"An unexpected error occured: {ex.Message}");
            }
        }
