...
namespace ECommerceApp.Services
{
    public class PaymentService
    {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    if (order.Payment != null)
                    {
                        ...
                        if (order.Payment.Status != PaymentStatus.Completed && order.OrderStatus == OrderStatus.Pending)
                        {
                    ...
                    if (!IsCashOnDelivery(paymentRequest.PaymentMethod))
                    {
                        ...
                        else
                        {
                            ...
                            return new ApiResponse<PaymentResponseDTO>(400, "Payment failed. Please try again.");
        ...
        public async Task<ApiResponse<PaymentResponseDTO>> GetPaymentByIdAsync(int id)
        {
            try
            {
            ...
            {
                return new ApiResponse<PaymentResponseDTO>(500, $"An unexpected error occured while retrieving the payment.");
            }
        }
        ...
        public async Task<ApiResponse<ConfirmationResponseDTO>> UpdatePaymentStatusAsync(PaymentStatusUpdateDTO statusUpdateDTO)
        {
            try
            {
                var payment = await _context.Payments
                    .Include(p => p.Order)
                    .FirstOrDefaultAsync(p => p.Id == statusUpdateDTO.PaymentId);
                ...
                if(statusUpdateDTO.Status == PaymentStatus.Completed && !IsCashOnDelivery(payment.PaymentMethod))
                {
        ...
        public async Task<ApiResponse<ConfirmationResponseDTO>> CompleteCODPaymentAsync(CODPaymentUpdateDTO codPaymentUpdateDTO )
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    {
                        return new ApiResponse<ConfirmationResponseDTO>(400, $"Ordered can't marked as Delivered from {payment.Order.OrderStatus} State ");
                    }

                    if(!IsCashOnDelivery(payment.PaymentMethod))
                    {
                        return new ApiResponse<ConfirmationResponseDTO>(409, "Payment method isn't Cash On Delivery.");
                    }
                ...
                {
                    await transaction.RollbackAsync();
                    return new ApiResponse<ConfirmationResponseDTO>(500, "An unexpected error occured while completing the COD payment.");
                }
            }
        }
        ...
        private bool IsCashOnDelivery(string paymentMehtod)
        {
            return paymentMehtod.Equals("CashOnDelivery", StringComparison.OrdinalIgnoreCase)
                || paymentMehtod.Equals("COD", StringComparison.OrdinalIgnoreCase);
        }
